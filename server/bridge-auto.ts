...
export function buildAutoPrompt(request: BridgeRequest, sceneJson: string): string {
  ...
  return [
    ...
    `- {"type":"remove_object","objectId":"<id of a furniture piece from a floor's objects[]>"}`,
    ...
    `REMOVE/CLEAR: to delete furniture, emit one remove_object per piece using its id from the floor's objects[] (each has id, category, roomId). To redesign or declutter a room, remove the unwanted pieces first, then place_furniture the replacements.`,
    ``,
